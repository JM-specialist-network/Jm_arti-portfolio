# Jm_arti-portfolio
# Hi, I'm Jose Miguel Artiles 👋

**Data Scientist** and **Economist-in-training** with a **Master's in Data Science for Business** (ENAE Business School) and a vocational **Administration and Finance** (Advanced Vocational Training degree, Spain). I focus on turning data into clear insights and decisions, combining quantitative analysis with business, financial and administrative experience. Skilled in R, SQL, Power BI and Python (using libraries such as Pandas, NumPy, Streamlit and others for data analysis and modelling).

**Científico de datos** y casi **economista** (por la Universidad de Murcia), con un **Máster en Data Science for Business** (ENAE Business School) y el título de **Técnico Superior en Administración y Finanzas (Ciclo Formativo de Grado Superior)**. Me centro en transformar los datos en información clara y decisiones fundamentadas, combinando análisis cuantitativo con experiencia de negocio, financiera y administrativa. Manejo R, SQL, Power BI y Python (utilizando librerías como Pandas, NumPy, Streamlit y otras para análisis y modelización de datos).

## 🛠️ Skills / Habilidades

- Programming / Lenguajes: Python, R, SQL  
- Python stack: Pandas, NumPy, scikit-learn, Streamlit (data apps), visualización básica  
- Data: limpieza de datos, EDA, visualización, reporting  
- BI & Reporting: cuadros de mando en Power BI, KPIs, informes ejecutivos  
- Web & Apps: dashboards y aplicaciones de datos con Streamlit  
- Domain / Dominio: economía, finanzas, administración y procesos de negocio. Experiencia con agentes de IA por el proyecto de mi TFM.

## 📊 Selected Projects / Proyectos destacados

| Type / Tipo | Project / Proyecto | Tech stack | Link |
|------------|--------------------|-----------|------|
| Python     | Pills from TFM – AI Financial Recommender with RAG | Python, Streamlit, LLMs, RAG, Pandas, NumPy | [Enlace](https://github.com/JM-specialist-network/tfm-financial-recommender-system) |
| Python     | Undergraduate Thesis (TFG) – Data Analysis Project | Python, data analysis, visualization | [Enlace](https://github.com/JM-specialist-network/TFG-Concentracion-Bancaria-Espana) |
| Python     | Santander Banking EDA | Python, Pandas, NumPy, Matplotlib, Seaborn, scikit-learn, statistical analysis | [Enlace](https://github.com/JM-specialist-network/python-santander-eda) |
| Python     | Deep Learning: Housing Price Prediction | Python, TensorFlow, Keras, neural networks, L1/L2/Dropout regularization, RMSE | [Enlace](https://github.com/JM-specialist-network/python-deep-learning-housing-prices) |


## 📁 Projects Overview / Resumen de proyectos

| Type / Tipo | Project / Proyecto | Tech stack | Link |
|------------|--------------------|-----------|------|
| R          | R Analytics & Time Series | R, tidyverse, time series, econometrics, macroeconomics | [Enlace](https://github.com/JM-specialist-network/r-finland-economy-quarto/)|
| R          | Movie Recommender System (Content-Based with TF-IDF) | R, tm, TF-IDF, cosine similarity, text mining, recommender systems | [Enlace](https://github.com/JM-specialist-network/r-movie-recommender-tfidf) |
| Power BI   | SuperStore Sales & Returns Dashboard | Power BI, DAX, data modelling, maps, sales & returns analytics | [Enlace](https://github.com/JM-specialist-network/powerbi-superstore) ||
| Power BI   | Marketing Analytics – Curso Datahack | Power BI, DAX, data modelling, dashboards | [Enlace](https://github.com/JM-specialist-network/powerbi-datahack-project/) |
| Dialogflow | Restaurant Booking Bot | Dialogflow ES, intents, entities, simple booking flow | [Enlace](https://github.com/JM-specialist-network/dialogflow-restaurant-booking-bot) |
| Python     | Spatial Analytics – Madrid Tourism (Sample) | Python, GeoPandas, Folium, spatial joins, choropleth maps, GIS | [Enlace](https://github.com/JM-specialist-network/python-gis-madrid-tourism) |


## 📫 Contact

- LinkedIn: [link_perfil](http://linkedin.com/in/jos%C3%A9-miguel-artiles-su%C3%A1rez-272491383)  
- Email: joseartiles@gmail.com



